...
{...}
{...}
{* *! svypd.sthlp version 1.01 - Biostat Global Consulting - 2018-04-11}{...}
{* Change log: }{...}
{* 				Updated}{...}
{*				version}{...}
...
{pstd} If the sample proportion is 0% or 100% and the user has requested Wald, Arcsine, 
       Logit or Wilson intervals, the program ignores the complex sampling design and 
	   returns a Clopper-Pearson interval calculated using the full sample size (assumes 
	   the design effect is 1).{p_end}

{pstd} If the standard error is 0, meaning that all clusters have the same observed coverage, 
       and the user has requested Wald, Arcsine, 
       Logit or Wilson intervals, the program ignores the complex sampling design and 
	   returns a Clopper-Pearson interval calculated using the full sample size (assumes 
	   the design effect is 1).{p_end}
...
{pstd}This command allows the user to calculate several types of CI, as suggested in Korn and 
...
{pstd} {bf:adjust} - If the user specifies the adjust option then the effective sample size for CI calculations will be 
		...
		D&P ({help svypd##2015:2015}) K&G ({help svypd##1998:1998}).{p_end}
...
{pstd}The program returns 11 scalar values: {p_end}
{p2colset 16 35 75 2}
...
{pstd} {bf:NOTE} If the sample proportion is 0% or 100% or if the standard error is 0, 
		...
		svypd will switch and calculate a Clopper-Pearson interval and return a value of 
		r(method) that says "Clopper-Pearson" instead of the method that the user specified.{p_end}
